...
-- Signature 
    data CatSig : Set where 

    data FuncSig : CatSig → CatSig → Set where 

    data ProFuncSig : CatSig → CatSig → Set where

    ProFuncSeq : List CatSig → Set 
    ProFuncSeq [] = CatSig
    ProFuncSeq (A ∷ []) = ⊤
    ProFuncSeq (A ∷ A' ∷ B) = ProFuncSig A A' × ProFuncSeq (A' ∷ B)
